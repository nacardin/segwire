...
impl NamespaceStateManager {
    ...
    pub async fn force_sync(&mut self, config_manager: &ConfigManager) -> SegwireResult<SyncResult> {
    ...
    pub async fn synchronize_state(&mut self, config_manager: &ConfigManager) -> SegwireResult<SyncResult> {
        ...
        let actual_namespaces = match self.netlink_manager.list_namespaces() {
            ...
            Err(e) => {
                ...
                result.errors.push(format!("Failed to list system namespaces: {}", e));
                return Ok(result);
            }
        };
        ...
        debug!("Found {} configured namespaces and {} actual managed namespaces", 
               namespace_configs.len(), managed_actual_namespaces.len());
        
        // Process each configured namespace
        for (config_name, config_entry) in namespace_configs {
            match self.process_configured_namespace(config_name, config_entry, &managed_actual_namespaces).await {
                Ok(action) => {
                    match action {
                ...
                Err(e) => {
                    error!("Failed to process configured namespace {}: {}", config_name, e);
                    result.errors.push(format!("Failed to process {}: {}", config_name, e));
                }
            }
        }
        
        // Process actual namespaces that don't have configuration
        for (actual_name, _actual_info) in &managed_actual_namespaces {
            if !namespace_configs.contains_key(actual_name) {
                debug!("Found actual namespace without configuration: {}", actual_name);
                
                let conflict = StateConflict {
                    namespace_name: actual_name.clone(),
                    conflict_type: ConflictType::NamespaceWithoutConfiguration,
                    description: format!("Namespace '{}' exists in system but has no configuration file", actual_name),
        ...
        for state_name in self.namespace_states.keys() {
            if !namespace_configs.contains_key(state_name) && !managed_actual_namespaces.contains_key(state_name) {
        ...
        match (current_state, actual_info) {
            ...
            (Some(_state), None) => {
                ...
                let conflict = StateConflict {
                    ...
                    description: format!("Namespace '{}' is tracked but doesn't exist in system", full_name),
    ...
    fn needs_namespace_update(
        ...
        let config_modified = config_entry.last_modified
            .duration_since(SystemTime::UNIX_EPOCH)
            .unwrap_or_default()
            .as_secs();
        
        if config_modified > current_state.last_updated {
            debug!("Configuration file newer than state for {}", current_state.name);
            return true;
        }
    ...
    async fn create_new_namespace(&mut self, config_entry: &NamespaceConfigEntry) -> SegwireResult<SyncAction> {
        let full_name = &config_entry.full_name;
    ...
    fn update_state_from_config_and_actual(
        ...
        state.routes = config_entry.config.routing.routes.iter().map(|route| {
    ...
    pub async fn resolve_conflict(&mut self, conflict: &StateConflict, config_manager: &ConfigManager) -> SegwireResult<()> {
        info!("Resolving conflict for namespace {}: {:?}", conflict.namespace_name, conflict.resolution);
        ...
        match conflict.resolution {
                ...
                if let Some(config_entry) = config_manager.get_namespace_config(&conflict.namespace_name) {
                    ...
                    return Err(SegwireError::Config(
                        segwire_common::error::ConfigError::InvalidValue {
                            ...
                            value: format!("No configuration found for {}", conflict.namespace_name),
                        }
                    ));
                }
            }
            ...
            ConflictResolution::DeleteNamespace => {
                ...
                match self.netlink_manager.delete_namespace(&conflict.namespace_name) {
                    ...
                    Err(e) => {
                        error!("Failed to delete namespace {}: {}", conflict.namespace_name, e);
                        return Err(e);
                    }
                }
            }
            ...
            ConflictResolution::FixConfiguration => {
                warn!("Configuration fix required for {}: {}", conflict.namespace_name, conflict.description);
                // This requires manual intervention - log the issue
                return Err(SegwireError::Config(
                    segwire_common::error::ConfigError::InvalidValue {
                        field: "configuration".to_string(),
                        value: format!("Manual configuration fix required: {}", conflict.description),
                    }
                ));
            }
            
            ConflictResolution::ManualIntervention => {
                warn!("Manual intervention required for {}: {}", conflict.namespace_name, conflict.description);
                return Err(SegwireError::Config(
                    segwire_common::error::ConfigError::InvalidValue {
...
"#,
    ...
    fn test_conflict_types() {
        ...
        assert_eq!(conflict.conflict_type, ConflictType::ConfigurationWithoutNamespace);
